...
fn c29_to_gfe(msg: &str) -> Option<Vec<Gfe>> {
    msg.as_bytes().iter().map(|&x| ascii_to_c29(x).map(|x| Gfe::from(x as i64))).collect::<Option<Vec<_>>>()
}

fn gfe_to_c29(msg: &[Gfe]) -> Option<String> {
    msg.iter().map(|&x| u8::try_from(*x).ok().and_then(|x| c29_to_ascii(x))).collect::<Option<Vec<u8>>>().map(|bytes| String::from_utf8(bytes).unwrap())
}
